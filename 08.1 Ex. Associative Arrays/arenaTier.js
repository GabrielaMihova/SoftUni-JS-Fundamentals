    ...
    while (command !== 'Ave Cesar') {
        ...
        if (action.length === 3) {
            ...
            if (gladiatorsPool.has(gladiatorA) && gladiatorsPool.has(gladiatorB)) {
                ...
                let bigSkillMap = techGladiatorA.size > techGladiatorB.size ?
                    techGladiatorA : techGladiatorB;
                let smallSkillMap = techGladiatorA.size > techGladiatorB.size ?
                    techGladiatorB : techGladiatorA
                for (let [techName, skill] of Array.from(bigSkillMap)) {
                    if (smallSkillMap.has(techName)) {
                        if (techGladiatorA.get(techName) >
                            techGladiatorB.get(techName)) {
